...
extension WaitingViewController {
    ...
    private func setupViewModel() {
        ...
        viewModel.getUserOrders(type: "waiting", page: 1)
    }
}
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let pagination = viewModel.pagination {
            if countOfOrders == indexPath.row + 1 && pagination.nextPage != nil {
                viewModel.delegate = self
                viewModel.getUserOrders(type: "waiting", page: pagination.nextPage ?? 1)
            }
        }
        
        cell.orderDescription.statusLabel.textColor = OrderTypes.processing.color
...
extension WaitingViewController : OrderViewModelProtocol {
    func getUserOrders() {
